...
import com.amall.core.web.virtual.CglibBean;
...
public class TransportTools {
	...
	public float cal_goods_trans_fee(String trans_id, String type,
		...
		TransportWithBLOBs trans = this.transportService.getByKey(CommUtil
				.null2Long(trans_id));
		String json = "";
		if (type.equals("mail")) {
	...
	public List<SysMap> query_cart_trans(StoreCart sc, String area_id) {
		...
		if ((area_id != null) && (!area_id.equals(""))) {
			Area area = this.areaService
					.getByKey(CommUtil.null2Long(area_id)).getParent();
			...
			for (GoodsCart gc : sc.getGcs()) {
				GoodsWithBLOBs goods = this.goodsService.getByKey(gc.getGoods()
						.getId());
					...
					if (goods.getTransport() != null) {
						mail_fee = mail_fee
								+ cal_order_trans(goods.getTransport()
										...
										.getTransport().getTransType(),
										goods.getGoodsWeight(),
										goods.getGoodsVolume(), city_name);
						...
						mail_fee = mail_fee
								+ CommUtil
										.null2Float(goods.getMailTransFee());
			...
			if ((mail_fee == 0.0F) && (express_fee == 0.0F)
					&& (ems_fee == 0.0F)) {
				sms.add(new SysMap("卖家承担", Integer.valueOf(0)));
			} else {
				sms.add(new SysMap("平邮[" + mail_fee + "元]", Float
						.valueOf(mail_fee)));
				sms.add(new SysMap("快递[" + express_fee + "元]", Float
						.valueOf(express_fee)));
				sms.add(new SysMap("EMS[" + ems_fee + "元]", Float
						.valueOf(ems_fee)));
			}
		}
		return sms;
	...
	public OrderForm dou(String orderId){
		OrderFormExample example = new OrderFormExample();
		example.clear();
		example.createCriteria().andOrderIdEqualTo(orderId.toString());
		OrderForm orderForm = orderFormService.getObjectList(example).get(0);
		return orderForm;
	} 
