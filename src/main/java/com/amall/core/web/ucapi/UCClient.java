...
public class UCClient extends PHPFunctions {
	...
	public static String UC_CLIENT_RELEASE = "20150612";
	...
	/**
	 * <p>Title: uc_api_post</p>
	 * <p>Description: </p>
	 * @param module  用户类型  user
	 * @param action  动作类型 
	 * @param arg    参数
	 * @return
	 */
	...
	public String uc_api_post(String module, String action,
		...
		for (String k : arg.keySet()) {
			Object v = (Map) arg.get(k);        //获得value
			Set<String> s1 = ((Map) v).keySet();     //设置key到set集合中
			k = urlencode(k);
	...
	/**
	 *  对第一个String类型的参数model的第一个字符进行判断 如果不是‘_’  就返回null，其他返回什么也不返回  
	 */
	...
	public String uc_authcode(String string, String operation, String key,
		...
		String keya = md5(substr(key, 0, 16));    //截取前16位
		String keyb = md5(substr(key, 16, 16));   //截取后16位    
		String keyc = ckey_length > 0 ? substr(md5(microtime()), -ckey_length)  //截取当前秒的后4位
				: operation.equals("DECODE") ? substr(string, 0, ckey_length)   //截取当前秒的前4位 
						: "";
	 ...
	 * 
	 * <p>Title: uc_app_ls</p>
	 * <p>Description: 判断是否是mysql方式链接 </p>
	 * @return
	 */
