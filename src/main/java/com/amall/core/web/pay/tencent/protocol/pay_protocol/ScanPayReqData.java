...
public class ScanPayReqData {
     ...
     */
    public ScanPayReqData(String body,String attach,String outTradeNo,int totalFee, String url){
        ...
        //订单生成的机器IP
        setSpbill_create_ip(Configure.SPBILL_CREATE_IP);

        //订单生成时间， 格式为yyyyMMddHHmmss，如2009年12 月25 日9 点10 分10 秒表示为20091225091010。时区为GMT+8 beijing。该时间取自商户服务器
        setTime_start(DateUtils.formatTimeByDate(new Date()));

        setTrade_type(Configure.TRADE_TYPE);
        
        setNotify_url(url + Configure.NOTIFY_URL);
        
        //订单失效时间，格式同上
        setTime_expire(DateUtils.formatTimeByDate(DateUtils.addMinutes(new Date(), 5)));
