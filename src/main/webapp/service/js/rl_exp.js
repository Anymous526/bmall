	...
	var rl_exp = {
		...
		loadImg:function(i){
			...
			for(var j = 0; j<rl_exp.num[i];j++){
				var domStr = 	'<li class="rl_exp_item">' + 
									'<img src="' + rl_exp.baseUrl + 'service/images/' + rl_exp.dir[i] + '/' + j + '.gif" alt="' + rl_exp.text[i][j] + 
		...
		init:function(){
			...
			$("#rl_exp_btn").bind('click',function(){
				...
				var w = $(this).position();
				$('#rl_bq').css({left:w.left,top:w.top+30}).fadeIn(200);
			});
			/*绑定关闭按钮*/
			$('#rl_bq a.closeface').bind('click',function(){
				$('#rl_bq').fadeOut(rl_exp.pace);
			});
